...
using IPHostEntry = Crestron.SimplSharp.IPHostEntry;
#if SSL
...
namespace uPLibrary.Networking.M2Mqtt
	...
	/// <param name="userCertificateValidationCallback">A LocalCertificateSelectionCallback delegate responsible for selecting the certificate used for authentication</param>
		...
		public MqttNetworkChannel (
			string remoteHostName, int remotePort, bool secure, X509Certificate caCert, X509Certificate clientCert, MqttSslProtocols sslProtocol)
#endif
			{
...
#if SSHARP
			var to = timeout * 1000;
			while (to-- > 0 && !this.socket.DataAvailable)
